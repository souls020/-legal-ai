...
import { z } from 'zod';
import { exportDocument, batchExport, getExportOptions, ExportFormat } from './export.service.js';
import { AuthRequest } from '../auth/auth.middleware.js';

// Validation schemas
const exportSchema = z.object({
  format: z.enum(['docx', 'pdf', 'txt']).default('docx'),
  includeHeader: z.coerce.boolean().optional(),
  includeFooter: z.coerce.boolean().optional(),
  pageNumbers: z.coerce.boolean().optional(),
  fontFamily: z.string().optional(),
  fontSize: z.coerce.number().min(8).max(72).optional(),
  lineHeight: z.coerce.number().min(1).max(3).optional()
});
...
export async function batchExportHandler(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: '未登录' });
...
// Get export options
export async function getExportOptionsHandler(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
    if (!userId) {
      return res.status(401).json({ success: false, message: '未登录' });
