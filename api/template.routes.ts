...
export async function listDocumentTypesHandler(req: Request, res: Response) {
  try {
    const db = await getDb();
    ...
    const types = dbAll<{ id: number; name: string; category: string; description: string }>(
      `SELECT id, name, category, description FROM document_types WHERE ${where} ORDER BY category, id`,
      params
    );
