...
export async function exportDocument(documentId: number, userId: number, options: ExportOptions): Promise<{
  success: boolean;
  filename?: string;
  content?: Buffer;
  mimeType?: string;
  ...
  const document = dbGet<Document>(
    'SELECT d.*, dt.name as type_name FROM documents d LEFT JOIN document_types dt ON d.type_id = dt.id WHERE d.id = ? AND d.user_id = ?',
    [documentId, userId]
  );
  ...
  // Track usage
  dbRun(
    'INSERT INTO usage_records (user_id, document_id, action_type, quota_used) VALUES (?, ?, ?, 0)',
    [userId, documentId, 'export']
  );
  await saveDb();
...
function exportAsText(document: Document & { type_name?: string | null }, filename: string): {
  success: boolean;
  filename: string;
  content: Buffer;
  mimeType: string;
...
function formatDocumentAsText(document: Document & { type_name?: string | null }): string {
  let content = '';
...
async function exportAsDocx(document: Document & { type_name?: string | null }, filename: string): Promise<{
  success: boolean;
  filename: string;
  content: Buffer;
  mimeType: string;
...
function formatDocumentAsHtml(document: Document & { type_name?: string | null }): string {
  let html = '';
...
function createHtmlDocx(document: Document & { type_name?: string | null }, bodyHtml: string): string {
...
async function exportAsPdf(document: Document & { type_name?: string | null }, filename: string): Promise<{
  success: boolean;
  filename: string;
  content: Buffer;
  mimeType: string;
...
function formatDocumentAsPrintHtml(document: Document & { type_name?: string | null }): string {
  const title = document.title || '法律文书';
  const date = new Date().toLocaleDateString('zh-CN');
...
export async function batchExport(documentIds: number[], userId: number, format: ExportFormat): Promise<{
  success: boolean;
  documents: Array<{ id: number; success: boolean; filename?: string; error?: string }>;
  error?: string;
}> {
  const results: Array<{ id: number; success: boolean; filename?: string; error?: string }> = [];
...
export async function getExportOptions(documentId: number, userId: number): Promise<{
