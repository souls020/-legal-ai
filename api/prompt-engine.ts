...
interface DocumentTypePrompt {
  structure: string;
  requirements: string;
  relatedLaws: string[];
}

const DOCUMENT_TYPE_PROMPTS: Record<string, DocumentTypePrompt> = {
  // 民事诉讼类
  '民事起诉状': {
    structure: `文书结构要求：
1. 标题：民事起诉状
...
const DEFAULT_DOCUMENT_PROMPT: DocumentTypePrompt = {
  structure: `文书结构要求：
1. 标题（文书名称）
2. 当事人/相关方信息
...
function getRelatedRegulations(lawNames: string[]): string {
  if (lawNames.length === 0) return '';

  const parts: string[] = [];
  for (const name of lawNames) {
    const reg = dbGet<{ title: string; content: string }>(
      'SELECT title, content FROM regulations WHERE title = ? AND status = ?',
      [name, 'effective']
    );
    if (reg) {
      parts.push(`\n### ${reg.title}（相关条文摘要）\n${reg.content}`);
...
export function buildDocumentGenerationMessages(
  typeId: number,
  caseInfo: Record<string, unknown>,
  conversationHistory?: ChatMessage[]
): ChatMessage[] {
  // Get document type info from DB
  const docType = dbGet<{ name: string; category: string; description: string }>(
    'SELECT name, category, description FROM document_types WHERE id = ?',
    [typeId]
  );
  ...
  const info = caseInfo as Record<string, string | undefined>;
  const contextParts: string[] = [];
  if (info.case_type) contextParts.push(`案件类型：${info.case_type}`);
  if (info.parties) contextParts.push(`当事人信息：${info.parties}`);
  if (info.facts) contextParts.push(`案件事实：${info.facts}`);
  if (info.claims) contextParts.push(`诉讼请求/申请事项：${info.claims}`);
  if (info.evidence) contextParts.push(`证据材料：${info.evidence}`);
