...
export async function generateStreamHandler(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, message: '未登录' });
        ...
        const result = await generateDocumentStream(userId, data.typeId, data.title, data.caseInfo, (chunk) => {
...
// Duplicate document
export async function duplicateDocumentHandler(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, message: '未登录' });
