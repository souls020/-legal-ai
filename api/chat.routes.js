import { z } from 'zod';
import { callAIChat, callAIChatStream } from './ai-provider.js';
...
export async function chatMessageHandler(req, res) {
    try {
        ...
        if (data.documentTypeId) {
            const docType = dbGet('SELECT name FROM document_types WHERE id = ?', [data.documentTypeId]);
            documentTypeName = docType?.name;
        }
        // Build messages with system prompt
        const messages = buildChatMessages(data.messages, documentTypeName);
...
export async function chatMessageStreamHandler(req, res) {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({ success: false, message: '未登录' });
