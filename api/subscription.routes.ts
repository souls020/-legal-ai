...
export async function changePlanHandler(req: AuthRequest, res: Response) {
  try {
    const userId = req.user?.id;
    if (!userId) return res.status(401).json({ success: false, message: '未登录' });
    ...
    if (existingSub) {
      ...
      db.run(
        `INSERT INTO subscriptions (user_id, plan_id, status, starts_at, expires_at, monthly_generations, used_generations, daily_generations, today_generations)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [userId, planId, 'active', now, expiresAt.toISOString(), 0, 0, 0, 0]
      );
    }

    // Get updated subscription
    const subscription = dbGet(
      `SELECT s.*, sp.name as plan_name, sp.features, sp.monthly_quota, sp.daily_quota
       FROM subscriptions s
       JOIN subscription_plans sp ON s.plan_id = sp.id
       WHERE s.user_id = ? AND s.status = 'active'`,
      [userId]
