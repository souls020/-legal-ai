...
import { getSubscription, listPlans, getUsage, changePlan, Subscription, SubscriptionPlan, UsageStats } from '../../services/subscription';
...
export const ProfilePage: React.FC = () => {
  ...
  const [usage, setUsage] = useState<UsageStats>({ total: 0, today: 0 });
  ...
  const handleChangePlan = async (planId: number) => {
    try {
      const newSub = await changePlan(planId);
      setSubscription(newSub);
      setPlanModalVisible(false);
      message.success('套餐更换成功');
    } catch (error) {
      message.error('套餐更换失败，请重试');
                ...
                <Col span={12}>
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <Title level={4} style={{ margin: '4px 0' }}>
                      {subscription?.plan_name || '免费套餐'}
                    </Title>
                  </div>
                  <div>
                    <Text type="secondary">有效期至</Text>
                    <div style={{ fontSize: 16 }}>
                      {subscription?.expires_at ? new Date(subscription.expires_at).toLocaleDateString('zh-CN') : '长期有效'}
                    </div>
                  </div>
                </Col>
                <Col span={12}>
                  <Button type="primary" size="large" onClick={() => setPlanModalVisible(true)}>
                    {subscription ? '更换套餐' : '开通套餐'}
                  </Button>
                </Col>
              </Row>
            </Card>
                ...
                <Col span={8}>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 32, fontWeight: 'bold', color: '#52c41a' }}>
                      {(subscription?.monthly_quota || 0) - (subscription?.used_generations || 0)}
                    </div>
                ...
                <Col span={8}>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 32, fontWeight: 'bold', color: '#722ed1' }}>
                      {subscription?.today_generations || usage.today}
                    </div>
              ...
              >
                <Row justify="space-between" align="middle">
                  <Col>
                    ...
                    <Text>
                      <Text strong>月度额度：</Text>
                      {plan.monthly_quota} 次/月
                    </Text>
                    <br />
                    <Text>
                      <Text strong>每日额度：</Text>
                      {plan.daily_quota} 次/日
                    </Text>
                    {features.length > 0 && (
                      <>
                        <Divider style={{ margin: '8px 0' }} />
                  ...
                  <Col>
                    <Title level={3} style={{ margin: 0, color: plan.price > 0 ? '#ff4d4f' : '#52c41a' }}>
                      {plan.price === 0 ? '免费' : `¥${plan.price}`}
                    </Title>
                    {plan.price > 0 && <Text type="secondary">/月</Text>}
                    <div style={{ marginTop: 16 }}>
                      {isCurrentPlan ? (
                        <Tag color="blue">当前套餐</Tag>
                      ) : (
                        <Button
                          type={plan.price > 0 ? 'primary' : 'default'}
                          onClick={() => handleChangePlan(plan.id)}
                        >
                          {plan.price > (subscription?.monthly_quota || 0) ? '升级' : '切换'}
                        </Button>
                      )}
                    </div>
                  </Col>
